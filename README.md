...
<img width="986" height="693" alt="スクリーンショット 2026-02-06 172859" src="https://github.com/user-attachments/assets/87dc52ae-d727-490a-9343-d3cc9953dd68" />
  ...
  - **多彩なメディア形式に対応**:
    ...
    - アニメーションGIF
    - ZIP書庫内の画像（見開き表示対応）
    - 動画: MP4, FLV（LibVLCによる再生）
    - 音声: MP3, WAV, FLAC（LibVLCによる再生）
    - PDF（見開き・ページ送り対応）
  - **再生コントロール**: 動画・音声共通の再生/一時停止、シークバー、音量調整
  - **表示モード切替**: 書庫・PDFは描画領域の右クリックで1ページ／見開き表示を切替可能（切替状態はファイル変更後も維持）
  - **ファイルドロップ**: ファイルをドラッグ＆ドロップで直接開ける

  ### サイドペイン（左側）
  - **移動先フォルダツリー（上段）**: 整理先フォルダを表示。複数ルート対応。右クリックでショートカットキー（Alt+1〜9）を割り当て可能
  - **移動元フォルダツリー（中段）**: 閲覧対象のフォルダを表示。複数ルート対応。フォルダ選択で自動的にメディアを読み込み
  - **ルート管理**: 各ツリーのルートフォルダを右クリックで管理（パス変更・ルート追加・ルート除外）
  ...
  - **状態の自動保存**: ツリーの展開状態・選択フォルダをアプリ終了時に保存し、次回起動時に復元
 ...
 |領域|説明|
|:----|:----|
|左上ツリー|移動先（整理先）フォルダ。複数ルート対応。ルート右クリックで管理（パス変更・追加・除外）。フォルダ右クリックでショートカットキー（Alt+1〜9）を割り当て可能|
|左中ツリー|移動元フォルダ。複数ルート対応。ルート右クリックで管理。フォルダを選択すると、その中の最初のメディアが表示される|
